                ...
                <form action="{{ route('updateproduct', ['product' => $product->id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-floating mb-3">
                        <select class="form-control" id="name_product" name="name_product" aria-describedby="floatingInputHelp">
                            @foreach ($productions as $production)
                                <option value="{{ $production->nama_product }}" {{ $product->name_product == $production->nama_product ? 'selected' : '' }}>
                    ...
                    <div class="form-floating mb-3">
                        <select class="form-control" id="stok" name="stok" aria-describedby="floatingInputHelp">
                            @foreach ($productions as $production)
                                <option value="{{ $production->jumlah_produksi }}" {{ $product->stok == $production->jumlah_produksi ? 'selected' : '' }}>
                    ...
                    <div class="form-floating mb-3">
                        <input type="text" class="form-control" id="harga_jual" name="harga_jual" value="{{ $product->harga_jual }}" placeholder="Rp. xxx.xxx" aria-describedby="floatingInputHelp" />
                        @error('harga_jual')
                            <span class="text-danger"> {{ $message }} </span>
                        @enderror
                    ...
                    <div class="mb-3">
                        <label for="existing-image" class="form-label">Existing Image</label>
                        @if ($product->gambar)
                            <img src="{{ asset('storage/' . $product->gambar) }}" alt="Existing Image" class="img-thumbnail" width="200">
                        @else
                    ...
                    <div class="row">
                        <div class="col-md-6 d-grid">
                            <a href="{{ route('product') }}" class="btn btn-outline-dark btn-lg mt-3"><i class="bi-arrow-left-circle me-2"></i> Cancel</a>
                        </div>
                        <div class="col-md-6 d-grid">
