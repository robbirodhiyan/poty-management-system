...
@section('content')
    ...
    <div>
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('updatedebit', ['debit' => $debit->id]) }}" method="POST">
                    @csrf
                    @method('put')
                    <fieldset disabled>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" id="name" name="name"
                                value="{{ $errors->any() ? old('name') : $debit->name }}" placeholder="John Doe"
                                aria-describedby="floatingInputHelp" />
                            <label for="floatingInput">Name</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" id="description" name="description"
                                value="{{ $errors->any() ? old('description') : $debit->description }}"
                                placeholder="Description" aria-describedby="floatingInputHelp" />
                            <label for="floatingInput">Description</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" id="total" name="total"
                                value="{{ $errors->any() ? old('total') : $debit->total }}" placeholder="Rp. xxxxxx"
                                aria-describedby="floatingInputHelp" />
                        ...
                        <div class="mb-3">
                            ...
                            <div class="col-md-10">
                                <input class="form-control" type="date"
                                    value="{{ $errors->any() ? old('date') : $debit->date }}"
                                    id="html5-date-input" />
                            </div>
                        </div>
                    </fieldset>
